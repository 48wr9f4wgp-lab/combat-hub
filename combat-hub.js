...
// v7.22.0-github — official-image/context/BOXING data hardening; visuals frozen

(async()=>{
const VERSION='7.22.0-github';
...
function fightContext(raw,eventName=''){let v=stripHTML(raw||'').replace(/\s+/g,' ').trim();if(!v)return'';if(eventIdentityText(v)===eventIdentityText(eventName))return'';if(/(?:live\s+on|watch\s+on|stream|broadcast|dazn|espn|paramount|fight\s*pass|youtube)/i.test(v))return'';const k=v.match(/-?\d+(?:\.\d+)?kg級/i);if(k)return k[0];const jp=v.match(/(?:女子)?(?:ストロー|フライ|バンタム|フェザー|ライト|ウェルター|ミドル|ライトヘビー|ヘビー)級(?:\s*(?:タイトルマッチ|マッチ))?/i);if(jp)return jp[0].replace(/\s+/g,' ');const en=v.match(/(?:Women's\s+)?(?:Strawweight|Flyweight|Bantamweight|Featherweight|Lightweight|Welterweight|Middleweight|Light Heavyweight|Heavyweight)\s+(?:Interim\s+)?(?:Title\s+)?Bout/i);if(en)return en[0];if(/\b(?:WBC|WBA|IBF|WBO|Ring)\b/i.test(v)&&/(?:title|championship|world)/i.test(v))return v.length>72?v.slice(0,72):v;return v.length<=48?v:'';}
...
function linkedFighterBouts(html,base){if(KEY!=='ufc'&&KEY!=='k1')return[];const scoped=cardScope(html),items=[],seen=new Set(),hrefOK=KEY==='ufc'?/\/athlete\/[^"'#?]+/i:/\/(?:k-1wgp\/)?fighter\/\d+/i;for(const m of scoped.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi)){const tag='<a '+m[1]+'>',href=attr(tag,'href')||'';if(!hrefOK.test(href))continue;const n=cardFighterName(m[2]),url=absoluteURL(href,base);if(!n||n.length<2||n.length>48||!url)continue;const key=eventIdentityText(n)+'|'+url;if(seen.has(key))continue;seen.add(key);items.push({name:n,url,index:m.index||0,end:(m.index||0)+m[0].length});}
  const out=[];for(let i=0;i+1<items.length&&out.length<8;i+=2){const a=items[i],b=items[i+1],from=Math.max(0,a.index-1800),to=Math.min(scoped.length,b.end+700),window=scoped.slice(from,to),plain=stripHTML(window);let ctx='';if(KEY==='k1'){const all=[...plain.matchAll(/-?\d+(?:\.\d+)?kg級(?:\s*\/[^\n]{0,80})?/gi)];ctx=all.length?fightContext(all[all.length-1][0]):'';}else{const all=[...plain.matchAll(/(?:女子)?(?:ストロー|フライ|バンタム|フェザー|ライト|ウェルター|ミドル|ライトヘビー|ヘビー)級\s*(?:タイトルマッチ|マッチ)|(?:Women's\s+)?(?:Strawweight|Flyweight|Bantamweight|Featherweight|Lightweight|Welterweight|Middleweight|Light Heavyweight|Heavyweight)\s+(?:Interim\s+)?(?:Title\s+)?Bout/gi)];ctx=all.length?fightContext(all[all.length-1][0]):'';}
...
function mainFromBout(data,p){const old=data?.main||{},context=fightContext(p?.context,data?.name)||fightContext(old.context,data?.name)||'';return{...old,a:p.a,b:p.b,context,aProfileURL:p.aProfileURL||old.aProfileURL||null,bProfileURL:p.bProfileURL||old.bProfileURL||null};}
...
function currentPagePairs(html,base=S.listing){if(!html)return[];if(KEY==='boxing'){const ring=ringDetailPairs(html);if(ring.length)return ring;}if(KEY==='one'){const out=[],seen=new Set(),chunks=String(html).split('<tr class="vs">').slice(1);for(const chunk of chunks){const row=chunk.split('</tr>')[0],titles=[...row.matchAll(/title="([^"]+)"/g)].map(m=>cleanName(m[1]));if(titles.length<2)continue;const a=titles[0],b=titles[1],k=a+'|'+b;if(a&&b&&!seen.has(k)){seen.add(k);out.push({a,b});}}if(out.length)return out.slice(0,6);}return officialPageBouts(html,base);}
async function refreshLockedCurrent(snap){const path=cacheFile(`combat-hub-current-${KEY}.json`),cached=readJSON(path),now=Date.now(),refreshTtl=KEY==='boxing'?2*3600000:30*60*1000;if(cached?.data&&now-Number(cached.savedAt)<refreshTtl)return{...snap,...cached.data,cardCheckedAt:Number(cached.savedAt)||null,cardRefreshedAt:Number(cached.savedAt)||null};try{const html=await reqText(snap.source,8),pairs=currentPagePairs(html,snap.source),images=sourceImageCandidates(html,snap.source,snap);let data={posterURL:images[0]?.url||null,posterSource:images[0]?.source||'none',posterCandidates:images.slice(0,8)};if(pairs.length&&sameFight(pairs[0].a,pairs[0].b,snap.main.a,snap.main.b)){const parsed=supportRowsFromPairs(snap,pairs),seen=new Set(parsed.map(r=>`${r.a}|${r.b}`));for(const r of (snap.support||[])){if(parsed.length>=5)break;const k=`${r.a}|${r.b}`;if(!seen.has(k)){seen.add(k);parsed.push(r);}}data={...data,main:mainFromBout(snap,pairs[0]),support:parsed,cardTba:false,cardSourceType:'official-detail'};}writeJSON(path,{savedAt:now,data});return{...snap,...data,cardCheckedAt:now,cardRefreshedAt:pairs.length?now:null};}catch(_){return cached?.data?{...snap,...cached.data,cardCheckedAt:Number(cached.savedAt)||null,cardRefreshedAt:Number(cached.savedAt)||null}:{...snap,cardSourceType:'trusted-snapshot'};}}
const KNOWN_EVENT_CARD_REFRESH_MS=30*60*1000,CARD_POLICY_VERSION=4;
...
async function loadData(){
  const snap=normalizeOneCompositeEvent({...SNAPSHOT[KEY]}),now=Date.now(),path=cacheFile(`combat-hub-next-${KEY}.json`),cached=readJSON(path),cachedData=cached?.data?normalizeOneCompositeEvent(cached.data):null;
  if(KEY==='boxing'){
    ...
    if(config.runsInWidget){
      ...
      return{...snap,startAt:new Date(now+24*3600000).toISOString(),location:'会場未定',name:'次大会',main:{a:'次大会',b:'確認中',context:''},support:[],cardTba:true,posterURL:null,source:S.listing,timeTba:true,displayDate:'日程未定',nextPending:true,lightweightPending:true,cacheVerified:false,cardSourceType:'safe-pending'};
    }
    if(currentLocked(snap)){
    ...
    return{...snap,cardTba:true,main:{a:'次大会',b:'確認中',context:''},support:[],nextPending:true,cacheVerified:false,cardSourceType:'safe-pending'};
  }
  ...
  return{...snap,cardTba:true,main:{a:'次大会',b:'確認中',context:''},support:[],nextPending:true,cardSourceType:'safe-pending'};
}
...
async function eventPosterResult(D,{localOnly=false}={}){const candidates=[];if(D?.posterURL)addImageCandidate(candidates,D.posterURL,D.posterSource||'data',D.source);for(const c of (D?.posterCandidates||[]))addImageCandidate(candidates,c.url,c.source||'data',D.source);let sourceError='';if(!localOnly&&D?.source){try{const h=await reqText(D.source,8);for(const c of sourceImageCandidates(h,D.source,D))addImageCandidate(candidates,c.url,c.source,D.source);}catch(_){sourceError='source_fetch_failed';}}
  let last='no_candidate';for(const c of candidates){const got=await cachedImageResult(c.url,`${KEY}-event`,!localOnly);if(got.image)return{image:got.image,url:c.url,source:c.source||'unknown',cacheHit:got.cacheHit,fallbackReason:null};last=got.error||'load_failed';}return{image:null,url:candidates[0]?.url||null,source:candidates[0]?.source||'none',cacheHit:false,fallbackReason:sourceError||last};}
