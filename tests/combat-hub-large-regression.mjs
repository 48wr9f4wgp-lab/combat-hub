...
assert.match(src, /function largeStatusHeading\(D\)\{if\(D\?\.nextPending\|\|D\?\.timeTba\)return'開催';/,'Medium/Large time-TBA and pending status must use 開催 rather than 開催まで');
assert.match(src, /return t>Date\.now\(\)\?'開催まで':'開催状況'/,'exact-time future events must retain 開催まで');
...
assert.match(src, /KEY==='boxing'\?'注目興行':'次大会'/,'BOXING Large provenance panel must use highlighted-event semantics');
