...
// v7.22.1-github — sanitize stale cached bout context; visuals frozen

(async()=>{
const VERSION='7.22.1-github';
...
function trustedMainContext(data){const n=NEXT_SNAPSHOT[KEY];if(!data?.main||!n?.main||!sameEventIdentity(data,n)||!sameFight(data.main.a,data.main.b,n.main.a,n.main.b))return'';return fightContext(n.main.context,n.name);}
function sanitizeEventFightContext(data){if(!data)return data;const main=data.main?{...data.main,context:fightContext(data.main.context,data.name)||trustedMainContext(data)||''}:data.main;const support=Array.isArray(data.support)?data.support.map(r=>({...r,context:fightContext(r.context,data.name)})):data.support;return{...data,main,support};}
...
const KNOWN_EVENT_CARD_REFRESH_MS=30*60*1000,CARD_POLICY_VERSION=5;
