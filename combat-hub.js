...
// v7.22.9-github — override unresolved placeholders with canonical verified metadata; visuals frozen

(async()=>{
const VERSION='7.22.9-github';
...
function sanitizeEventFightContext(data){if(!data)return data;const trusted=trustedMainContext(data),main=data.main?{...data.main,context:richerContext(data.main.context,trusted,data.name)}:data.main;const support=Array.isArray(data.support)?data.support.map((r,i)=>({...r,label:normalizedExistingSupportLabel(r.label,i),context:fightContext(r.context,data.name)})):data.support;return{...data,main,support};}
...
const KNOWN_EVENT_CARD_REFRESH_MS=30*60*1000,CARD_POLICY_VERSION=9,LARGE_NEXT_POLICY_VERSION=3;
