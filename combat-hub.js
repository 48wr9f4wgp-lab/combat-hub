...
function earlierTrustedEvent(base,other,kind='rollforward',now=Date.now()){if(KEY!=='k1'||!other)return null;const eligible=kind==='next'?nextEligible:rollforwardEligible,n=trustedFutureEvents().find(x=>eligible(base,x,now));if(!n)return null;const nt=new Date(n.startAt).getTime(),ot=new Date(other?.startAt||0).getTime();if(!Number.isFinite(ot)||ot<=0||sameEventIdentity(n,other)||nt>=ot)return null;return enrichTrustedEventMeta(normalizeOneCompositeEvent({...n}));}
