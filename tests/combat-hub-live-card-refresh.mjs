...
assert.match(src,/const VERSION='7\.23\.7-github'/);
assert.match(src,/const KNOWN_EVENT_CARD_REFRESH_MS=30\*60\*1000,CARD_POLICY_VERSION=10,LARGE_NEXT_POLICY_VERSION=3/);
...

{
  ...
  assert.equal(api.normalizedExistingSupportLabel('MAIN CARD',0),'CO-MAIN','first cached support role must normalize to CO-MAIN');
