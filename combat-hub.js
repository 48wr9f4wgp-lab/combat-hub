...
// v7.23.7-github — prefer earlier verified events over stale later event caches; visuals frozen

(async()=>{
const VERSION='7.23.7-github';
...
const BOXING_SOURCE_POLICY_VERSION=3;
const BOXING_OFFICIAL_SOURCES=[
  ...
  {id:'teiken',label:'帝拳',url:'https://www.teiken.com/bout/'}
];
// Golden Boy is intentionally excluded until a stable public schedule surface is verified.
const BOXING_TRUSTED_HIGHLIGHTS=[
  {startAt:'2026-09-27T16:30:00+09:00',displayDate:'9/27 (日)',timeTba:false,location:'TOYOTA ARENA TOKYO',name:'Prime Video Boxing 16',source:'https://www.teiken.com/bout/',sourceId:'teiken',promoter:'帝拳',aliases:{a:['Takuma Inoue','Inoue'],b:['Tenshin Nasukawa','Nasukawa']},main:{a:'井上拓真',b:'那須川天心',context:'WBC世界バンタム級タイトルマッチ'},support:[{label:'CO-MAIN',a:'西田凌佑',b:'サム・グッドマン',context:'IBF世界スーパーバンタム級暫定王座決定戦'},{label:'MAIN CARD',a:'坪井智也',b:'リカルド・マラジカ',context:'WBC世界スーパーフライ級王座決定戦'}],cardTba:false,verifiedFallback:true,localPromoterTruth:true},
...
const TRUSTED_FUTURE={
  ...
  one:[{startAt:'2026-09-25T22:30:00+09:00',location:'バンコク',name:'ONE Friday Fights 172',source:'https://www.onefc.com/events/one-friday-fights-172/',timeTba:false,main:{a:'Panpayak Jitmuangnon',b:'Lamnamoonlek Torfunfarm',context:'フライ級キックボクシング'},support:[],cardTba:false}],
  k1:[
    {startAt:'2026-09-27T07:00:00+09:00',displayDate:'9/27 (日)',location:'ブラジル・ブラジリア / SOCIAL HALL AABB BRASILIA',name:'K-1 WORLD GP 2026 -90KG in BRASILIA',source:'https://www.k-1.co.jp/schedule/16689',timeTba:false},
    {startAt:'2026-11-23T00:00:00+09:00',displayDate:'11/23 (月・祝)',location:'後楽園ホール',name:'K-1 2026.11.23',source:'https://www.k-1.co.jp/schedule/16670',timeTba:true},
    {startAt:'2026-12-29T00:00:00+09:00',displayDate:'12/29 (火)',location:'横浜BUNTAI',name:'K-1 2026.12.29',source:'https://www.k-1.co.jp/schedule/16678',timeTba:true}
  ]
};
...
function trustedSourceKey(ev){let v=eventSourceKey(ev);if(KEY==='ufc')v=v.replace(/^jp\./,'');if(KEY==='k1')v=v.replace('/k-1wgp/schedule/','/schedule/');return v;}
function eventJstDateKey(ev){const t=new Date(ev?.startAt||0).getTime();if(!Number.isFinite(t)||t<=0)return'';const d=new Date(t+9*3600000);return `${d.getUTCFullYear()}-${String(d.getUTCMonth()+1).padStart(2,'0')}-${String(d.getUTCDate()).padStart(2,'0')}`;}
function trustedFutureMatch(ev){if(!ev)return null;const src=trustedSourceKey(ev),date=eventJstDateKey(ev);return trustedFutureEvents().find(x=>{const xs=trustedSourceKey(x),xd=eventJstDateKey(x);if(src&&xs&&src===xs)return true;if(date&&xd&&date===xd&&validOrgName(ev.name||x.name))return true;return sameEventIdentity(ev,x);})||null;}
function unresolvedLocation(v){const s=stripHTML(v||'').trim();return !s||/^(?:会場未定|会場確認中|確認中|未定)$/i.test(s);}
function contextScore(v){const s=fightContext(v||'');if(!s)return 0;let score=1;if(/Kickboxing|キックボクシング|Muay Thai|ムエタイ|\bMMA\b/i.test(s))score+=4;if(/タイトル|Title|Championship|WBC|WBA|IBF|WBO|Ring/i.test(s))score+=3;if(/-?\d+(?:\.\d+)?kg級|ストロー|フライ|バンタム|フェザー|ライト|ウェルター|ミドル|ヘビー/i.test(s))score+=1;return score;}
function richerContext(a,b,eventName=''){const aa=fightContext(a,eventName),bb=fightContext(b,eventName);return contextScore(bb)>contextScore(aa)?bb:(aa||bb||'');}
function enrichTrustedEventMeta(ev){if(!ev)return ev;const t=trustedFutureMatch(ev);if(!t)return ev;const rawName=stripHTML(ev.name||''),generic=KEY==='ufc'&&genericEventIdentityName(rawName),liveLoc=shortLoc(ev.location||''),trustedLoc=shortLoc(t.location||''),hasMain=!!(ev.main?.a&&ev.main?.b&&ev.main.a!=='対戦カード'&&ev.main.b!=='発表待ち'),sameMain=!!(hasMain&&t.main&&sameFight(ev.main.a,ev.main.b,t.main.a,t.main.b)),main=hasMain?{...ev.main,context:sameMain?richerContext(ev.main.context,t.main.context,ev.name):fightContext(ev.main.context,ev.name)}:(t.main?{...t.main}:ev.main);return{...ev,name:generic||!rawName?(t.name||ev.name):ev.name,location:unresolvedLocation(liveLoc)?(trustedLoc||liveLoc):liveLoc,displayDate:ev.displayDate||t.displayDate||null,timeTba:typeof ev.timeTba==='boolean'?ev.timeTba:!!t.timeTba,main,support:(ev.support||[]).length?ev.support:(t.support?[...t.support]:ev.support),cardTba:hasMain?!!ev.cardTba:(t.main?false:!!ev.cardTba)};}
...
'Takuma Inoue':'井上拓真','Tenshin Nasukawa':'那須川天心','Ryosuke Nishida':'西田凌佑','Sam Goodman':'サム・グッドマン','Tomoya Tsuboi':'坪井智也','Ricardo Malajika':'リカルド・マラジカ',
...
function sanitizeFighter(s){let v=cleanName(s);if(KEY==='ufc'){v=v.replace(/^UFC[^|]{0,120}\|\s*/i,'').replace(/^UFC Fight Night:\s*/i,'').replace(/^UFC\s+\d+\s*[:：]\s*/i,'').replace(/\s*\|\s*UFC.*$/i,'').trim();}return v;}
...
function shortLoc(s){const v=String(s||'');if(/TOYOTA\s*ARENA\s*TOKYO|トヨタアリーナ東京/i.test(v))return'トヨタアリーナ東京';if(/LaLa\s*arena\s*TOKYO-BAY|ららアリーナ\s*東京ベイ/i.test(v))return'千葉・船橋';if(/Shanghai|Pudong|China|上海/i.test(v))return'上海';if(/Osaka|大阪/i.test(v))return'大阪';if(/Bangkok|Lumpinee|バンコク|ルンピニー/i.test(v))return'バンコク';if(/Paris|パリ/i.test(v))return'パリ';if(/Las Vegas|ラスベガス/i.test(v))return'ラスベガス';if(/San Diego|サンディエゴ/i.test(v))return'サンディエゴ';if(/Brasilia|BRASILIA|ブラジリア/i.test(v))return'ブラジル・ブラジリア';if(/Yokohama Buntai|Yokohama BUNTAI|横浜BUNTAI|Yokohama|横浜/i.test(v))return'横浜BUNTAI';if(/代々木|Yoyogi/i.test(v))return'東京・代々木第二';if(/Tokyo|東京/i.test(v))return'東京';return v.length>14?v.slice(0,13)+'…':v;}
...
function boxingSourceId(v){
  ...
  if(/teiken\.com/.test(s))return'teiken';
  return'';
}
function boxingPromoter(v){const id=boxingSourceId(v);return BOXING_OFFICIAL_SOURCES.find(x=>x.id===id)?.label||'';}
function boxingSourcePriority(e){return({teiken:120,matchroom:110,pbc:110,toprank:110,queensberry:110,ring:70})[e?.sourceId||boxingSourceId(e)]||0;}
...
function boxingCleanFighterName(v){return cleanName(v).replace(/\s+(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:t(?:ember)?)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\.?$/i,'').replace(/\s+(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)\.?$/i,'').trim();}
function boxingPairFromText(text){
  ...
  return m?{a:boxingCleanFighterName(m[1]),b:boxingCleanFighterName(m[2])}:null;
}
function boxingOfficialListingEvents(html,cfg,now=Date.now()){
  ...
  if(cfg.id==='ring')for(const e of ringListingEvents(raw,cfg.url,now))add({...e,sourceId:cfg.id,promoter:cfg.label,liveSource:true,main:e.main||boxingPairFromText(e.name)});
...
function boxingHighlightScore(e,now=Date.now()){
  ...
  if(e.liveSource)s+=10;if(e.localPromoterTruth)s+=12;if(e.sourceId==='ring')s+=4;s+=Math.min(6,(e.verifiedSources||[]).length*2);return s;
}
...
function boxingAliasSet(e,side){const vals=[e?.main?.[side],...(e?.aliases?.[side]||[])].map(boxingCleanFighterName).filter(Boolean),out=new Set();for(const v of vals){out.add(fighterIdentity(v));out.add(eventIdentityText(v));}return out;}
function boxingAliasMatch(a,b,sideA,sideB){const A=boxingAliasSet(a,sideA),B=boxingAliasSet(b,sideB);for(const x of A)if(x&&B.has(x))return true;return false;}
function boxingCandidateMatch(a,b){if(!a||!b||eventJstDateKey(a)!==eventJstDateKey(b))return false;return (boxingAliasMatch(a,b,'a','a')&&boxingAliasMatch(a,b,'b','b'))||(boxingAliasMatch(a,b,'a','b')&&boxingAliasMatch(a,b,'b','a'));}
function mergeBoxingCandidate(a,b){
  if(!a)return b;if(!b)return a;const ap=boxingSourcePriority(a),bp=boxingSourcePriority(b),base=bp!==ap?(bp>ap?b:a):(boxingCandidateQuality(b)>boxingCandidateQuality(a)?b:a),other=base===a?b:a,visualSource=base.visualSource||(boxingSourcePriority(other)<boxingSourcePriority(base)?other.source:null);return{...base,visualSource:visualSource||null,posterURL:base.posterURL||other.posterURL||null,posterSource:base.posterSource||other.posterSource||'none',posterCandidates:(base.posterCandidates||[]).length?base.posterCandidates:(other.posterCandidates||[]),liveSource:!!(a.liveSource||b.liveSource),verifiedFallback:!!(a.verifiedFallback||b.verifiedFallback),verifiedSources:[...new Set([...(a.verifiedSources||[a.sourceId].filter(Boolean)),...(b.verifiedSources||[b.sourceId].filter(Boolean))])],location:base.location||other.location||'',main:{...(base.main||other.main),context:richerContext(base.main?.context,other.main?.context,base.name||other.name)},support:(base.support||[]).length?base.support:(other.support||[])};
}
function dedupeBoxingCandidates(candidates){
  const out=[];for(const e of candidates){if(!boxingHighlightValid(e))continue;const i=out.findIndex(x=>boxingCandidateMatch(x,e));if(i>=0)out[i]=mergeBoxingCandidate(out[i],e);else out.push(e);}return out;
}
async function discoverBoxingHighlight(now=Date.now()){
  ...
  candidates=dedupeBoxingCandidates(candidates);
  const future=candidates.filter(e=>new Date(e.startAt).getTime()>now);
  if(future.length)candidates=future;
  candidates.sort((a,b)=>boxingHighlightScore(b,now)-boxingHighlightScore(a,now)||new Date(a.startAt)-new Date(b.startAt));
  return candidates[0]?{...sanitizeEventFightContext(candidates[0]),highlight:true,highlightScore:boxingHighlightScore(candidates[0],now),cardSourceType:'official-highlight'}:null;
}
...
function trustedMainContext(data){if(!data?.main)return'';const n=trustedFutureMatch(data);if(!n?.main||!sameFight(data.main.a,data.main.b,n.main.a,n.main.b))return'';return fightContext(n.main.context,n.name);}
function sanitizeEventFightContext(data){if(!data)return data;const cleanBout=r=>!r?r:(KEY==='ufc'?{...r,a:sanitizeFighter(r.a),b:sanitizeFighter(r.b)}:{...r}),baseMain=cleanBout(data.main),trusted=trustedMainContext({...data,main:baseMain}),main=baseMain?{...baseMain,context:richerContext(baseMain.context,trusted,data.name)}:baseMain;const support=Array.isArray(data.support)?data.support.map((r,i)=>{const q=cleanBout(r);return{...q,label:normalizedExistingSupportLabel(q.label,i),context:fightContext(q.context,data.name)};}):data.support;return{...data,main,support};}
...
const KNOWN_EVENT_CARD_REFRESH_MS=30*60*1000,CARD_POLICY_VERSION=10,LARGE_NEXT_POLICY_VERSION=3;
...
function earlierTrustedEvent(base,other,kind='rollforward',now=Date.now()){if(KEY!=='k1')return null;const eligible=kind==='next'?nextEligible:rollforwardEligible,n=trustedFutureEvents().find(x=>eligible(base,x,now));if(!n)return null;const nt=new Date(n.startAt).getTime(),ot=new Date(other?.startAt||0).getTime();if(other&&Number.isFinite(ot)&&ot>0&&(sameEventIdentity(n,other)||nt>=ot))return null;return enrichTrustedEventMeta(normalizeOneCompositeEvent({...n}));}
function boxingOfficialSource(e){return!!boxingSourceId(e);}
function boxingRingSource(e){return boxingSourceId(e)==='ring';}
function boxingPrefetchValid(snap,e,now=Date.now()){return KEY==='boxing'&&!!e&&boxingRingSource(e)&&rollforwardEligible(snap,e,now);}
function boxingVerifiedCache(cached,snap,now){if(KEY!=='boxing'||!cached?.data||cached?.verifiedBy!=='boxingHighlightDiscovery'||Number(cached?.sourcePolicy)!==BOXING_SOURCE_POLICY_VERSION||!Number.isFinite(Number(cached?.verifiedAt)))return null;const data=sanitizeEventFightContext(normalizeOneCompositeEvent(cached.data));return boxingHighlightValid(data,now)?{...data,highlight:true}:null;}
function boxingCurrentVerifiedCache(cached,snap){if(KEY!=='boxing'||!cached?.data||cached?.verifiedBy!=='refreshLockedCurrent'||!Number.isFinite(Number(cached?.verifiedAt)))return null;const data=normalizeOneCompositeEvent(cached.data);return currentLocked(snap)&&boxingRingSource(data)&&sameEventIdentity(data,snap)?data:null;}
async function prefetchBoxingVisual(data){if(KEY!=='boxing'||!boxingOfficialSource(data))return data;let last=null;for(const source of [...new Set([data.visualSource,data.source].filter(Boolean))]){const p=await eventPosterResult({...data,source},{localOnly:false});last=p;if(p.image)return{...data,posterURL:p.url||data.posterURL||null,posterSource:p.source||data.posterSource||'none',posterPrefetched:true,visualSource:source};}return{...data,posterURL:last?.url||data.posterURL||null,posterSource:last?.source||data.posterSource||'none',posterPrefetched:false};}
function safePendingEvent(now=Date.now(),lightweight=false){const box=KEY==='boxing',name=box?'注目興行':'次大会';return{startAt:new Date(now+24*3600000).toISOString(),location:'会場未定',name,main:{a:name,b:'確認中',context:''},support:[],cardTba:true,posterURL:null,posterCandidates:[],posterSource:'none',source:null,timeTba:true,displayDate:'日程未定',nextPending:true,highlightPending:box,lightweightPending:!!lightweight,cacheVerified:false,cardSourceType:box?'highlight-pending':'safe-pending'};}
async function loadData(){
  const snap=sanitizeEventFightContext(normalizeOneCompositeEvent({...SNAPSHOT[KEY]})),now=Date.now(),path=cacheFile(`combat-hub-next-${KEY}.json`),cached=readJSON(path),cachedData=cached?.data?sanitizeEventFightContext(enrichTrustedEventMeta(normalizeOneCompositeEvent(cached.data))):null;
  if(KEY==='boxing'){
    ...
    if(config.runsInWidget){
      ...
      if(verifiedFuture)return{...verifiedFuture,prefetched:true,cacheVerified:true,cardSourceType:'verified-boxing-highlight-cache',cardCheckedAt:Number(cached?.savedAt)||null,cardRefreshedAt:Number(cached?.verifiedAt)||null};
    ...
    const live=await discoverBoxingHighlight(now);
    if(live&&boxingHighlightValid(live,now)){const verified=await prefetchBoxingVisual(live);writeJSON(path,{savedAt:now,verifiedAt:now,verifiedBy:'boxingHighlightDiscovery',sourcePolicy:BOXING_SOURCE_POLICY_VERSION,data:verified});return{...verified,prefetched:true,cacheVerified:true,cardCheckedAt:now,cardRefreshedAt:now,cardSourceType:'official-highlight'};}
    if(verifiedFuture)return{...verifiedFuture,stale:true,prefetched:true,cacheVerified:true,cardSourceType:'verified-boxing-highlight-cache',cardCheckedAt:Number(cached?.savedAt)||null,cardRefreshedAt:Number(cached?.verifiedAt)||null};
...
function trustedLargeNext(base){const n=trustedFutureEvents().find(x=>nextEligible(base,x,Date.now()));return n?enrichTrustedEventMeta(normalizeOneCompositeEvent({...n})):null;}
async function loadLargeNext(base){const path=cacheFile(`combat-hub-large-next-${KEY}.json`),cached=readJSON(path),now=Date.now(),cachedData=cached?.data?enrichTrustedEventMeta(normalizeOneCompositeEvent(cached.data)):null,earlierTrustedCached=earlierTrustedEvent(base,cachedData,'next',now);if(cachedData&&Number(cached?.policy)===LARGE_NEXT_POLICY_VERSION&&now-Number(cached.savedAt)<6*3600000&&nextEligible(base,cachedData,now)&&!earlierTrustedCached)return cachedData;if(earlierTrustedCached){writeJSON(path,{savedAt:now,policy:LARGE_NEXT_POLICY_VERSION,data:earlierTrustedCached});return earlierTrustedCached;}try{const listing=await reqText(S.listing,7),eligible=e=>nextEligible(base,e,now);let candidates=jsonLdEvents(listing,S.listing).map(normalizeOneCompositeEvent).filter(eligible);if(KEY==='ufc')candidates.push(...ufcListingEvents(listing,S.listing,Math.max(new Date(base.startAt).getTime()+6*3600000,now),now+180*86400000).filter(eligible));if(KEY==='k1')candidates.push(...k1ListingEvents(listing,S.listing,now).filter(eligible));const uniq=new Map();for(const e of candidates){const k=`${eventSourceKey(e)}|${eventIdentityText(e.name)}|${new Date(e.startAt).getTime()}`;if(!uniq.has(k))uniq.set(k,e);}candidates=[...uniq.values()].filter(eligible).sort((a,b)=>new Date(a.startAt)-new Date(b.startAt));if(!candidates.length){const deep=await strictNextEvent(base);if(deep&&eligible(deep)){const data={name:stripHTML(deep.name)||S.label,startAt:deep.startAt,location:shortLoc(deep.location||''),source:deep.source||S.listing,timeTba:!!deep.timeTba,displayDate:deep.displayDate||null};writeJSON(path,{savedAt:now,policy:LARGE_NEXT_POLICY_VERSION,data});return data;}const trusted=trustedLargeNext(base);if(trusted){writeJSON(path,{savedAt:now,policy:LARGE_NEXT_POLICY_VERSION,data:trusted});return trusted;}return cachedData&&Number(cached?.policy)===LARGE_NEXT_POLICY_VERSION&&eligible(cachedData)?cachedData:null;}const ev=enrichTrustedEventMeta(candidates[0]),data=enrichTrustedEventMeta({name:stripHTML(ev.name)||S.label,startAt:ev.startAt,location:shortLoc(ev.location||''),source:ev.source||S.listing,timeTba:!!ev.timeTba,displayDate:ev.displayDate||null,main:ev.main,support:ev.support,cardTba:ev.cardTba});writeJSON(path,{savedAt:now,policy:LARGE_NEXT_POLICY_VERSION,data});return data;}catch(_){return trustedLargeNext(base)||(cachedData&&Number(cached?.policy)===LARGE_NEXT_POLICY_VERSION&&nextEligible(base,cachedData,now)?cachedData:null);}}
...
function writeRuntimeAudit(D,ctx){try{writeJSON(cacheFile('combat-hub-runtime-audit.json'),{savedAt:Date.now(),version:VERSION,loaderVersion:typeof LOADER_VERSION!=='undefined'?LOADER_VERSION:null,key:KEY,widgetFamily:config.widgetFamily||null,nextPending:!!D.nextPending,highlightPending:!!D.highlightPending,highlight:!!D.highlight,highlightScore:D.highlightScore??null,promoter:D.promoter||null,sourceId:D.sourceId||null,verifiedSources:D.verifiedSources||null,lightweightPending:!!D.lightweightPending,cardTba:!!D.cardTba,lockedCurrent:!!D.lockedCurrent,prefetched:!!D.prefetched,cacheVerified:!!D.cacheVerified,name:D.name||null,source:D.source||null,imageMode:ctx?.imageMode||'gradient',posterURLResolved:ctx?.posterURLResolved||null,posterSource:ctx?.posterSource||'none',posterLoaded:!!ctx?.poster,aProfileURL:ctx?.a?.profileURL||D.main?.aProfileURL||null,bProfileURL:ctx?.b?.profileURL||D.main?.bProfileURL||null,aImageLoaded:!!ctx?.a?.image,bImageLoaded:!!ctx?.b?.image,imageCacheHit:!!ctx?.imageCacheHit,imageFallbackReason:ctx?.imageFallbackReason||null,cardSourceType:D.cardSourceType||null,cardCheckedAt:D.cardCheckedAt||null,cardRefreshedAt:D.cardRefreshedAt||null});}catch(_){}}
...
function division(s){const v=String(s||''),title=/タイトル|TITLE|CHAMPIONSHIP/i.test(v),disc=/Kickboxing|キックボクシング/i.test(v)?'キックボクシング':/Muay Thai|ムエタイ/i.test(v)?'ムエタイ':/\bMMA\b/i.test(v)?'MMA':'',plain=x=>disc?x+disc:x;if(/FLY|フライ/i.test(v))return title?'フライ級タイトル戦':plain('フライ級');if(/FEATHER|フェザー/i.test(v))return title?'フェザー級タイトル戦':plain('フェザー級');if(/BANTAM|バンタム/i.test(v))return/スーパー|SUPER/i.test(v)?'スーパー・バンタム級':title?'バンタム級タイトル戦':plain('バンタム級');if(/STRAW|ストロー/i.test(v))return title?'ストロー級タイトル戦':plain('ストロー級');if(/LIGHT HEAVY|ライトヘビー/i.test(v))return title?'ライトヘビー級タイトル戦':plain('ライトヘビー級');if(/HEAVY|ヘビー/i.test(v))return title?'ヘビー級タイトル戦':plain('ヘビー級');if(/MIDDLE|ミドル/i.test(v))return title?'ミドル級タイトル戦':plain('ミドル級');if(/WELTER|ウェルター/i.test(v))return title?'ウェルター級タイトル戦':plain('ウェルター級');if(/LIGHT|ライト/i.test(v))return title?'ライト級タイトル戦':plain('ライト級');return v.length>25?v.slice(0,24)+'…':v;}
...
function renderLarge(w,D,ctx,next,nextPoster){
  ...
  if(pending){
    ...
    const title=tx(center,D.nextPending?(KEY==='boxing'?'注目興行を確認中':'次大会情報を確認中'):'対戦カード発表待ち',LARGE_UI.pending,new Color(C.text),'black');title.centerAlignText();
    center.addSpacer(7);
  ...
  const ncap=right.addStack();tx(ncap,KEY==='boxing'?'注目興行':'次大会',LARGE_UI.nextLabel,new Color(S.accent),'bold');ncap.addSpacer();
  if(KEY==='boxing'){right.addSpacer(7);const src=tx(right,D.promoter||'複数公式',LARGE_UI.nextTitle,new Color(C.text),'bold',2);src.minimumScaleFactor=.72;right.addSpacer(7);const verified=tx(right,D.cacheVerified?'公式確認済み':'公式情報を確認中',LARGE_UI.nextMeta,D.cacheVerified?new Color(C.sub):new Color(C.muted),'semibold',2);verified.minimumScaleFactor=.72;right.addSpacer();if(D.cacheVerified){const nt=tx(right,D.timeTba?'時刻未定':'あと '+countdown(D),LARGE_UI.nextCountdown,new Color(S.accent),'bold');nt.minimumScaleFactor=.72;}}
  else if(next){right.addSpacer(7);const nn=tx(right,largeNextTitle(next),LARGE_UI.nextTitle,new Color(C.text),'bold',2);nn.minimumScaleFactor=.74;right.addSpacer(7);const nm=tx(right,largeNextMeta(next),LARGE_UI.nextMeta,new Color(C.sub),'semibold',2);nm.minimumScaleFactor=.74;right.addSpacer();const nt=tx(right,next.timeTba?'時刻未定':'あと '+countdown(next),LARGE_UI.nextCountdown,new Color(S.accent),'bold');nt.minimumScaleFactor=.72;}
...
function renderSmall(w,D,ctx){
  ...
  w.addSpacer(7);
  if(pending){
    const title=tx(w,D.nextPending?(KEY==='boxing'?'注目興行\n確認中':'次大会情報\n確認中'):'対戦カード\n発表待ち',SMALL_UI.pending,new Color(C.text),'black',2);title.minimumScaleFactor=.72;title.centerAlignText();
    w.addSpacer(4);
    const sub=tx(w,D.nextPending?(KEY==='boxing'?'検証済み情報のみ表示':'公式発表を待機中'):'公式カード更新待ち',SMALL_UI.pendingSub,new Color(S.accent),'semibold',1);sub.minimumScaleFactor=.70;sub.centerAlignText();
  }else{
...
function renderMedium(w,D,ctx){
  ...
  if(pending){
    const center=w.addStack();center.layoutVertically();
    const title=tx(center,D.nextPending?(KEY==='boxing'?'注目興行を確認中':'次大会情報を確認中'):'対戦カード発表待ち',MEDIUM_UI.pending,new Color(C.text),'black',1);title.minimumScaleFactor=.76;
    center.addSpacer(5);
    const sub=tx(center,D.nextPending?(KEY==='boxing'?'検証済み情報のみ表示':'公式発表を待機中'):'公式カード更新待ち',MEDIUM_UI.pendingSub,new Color(S.accent),'semibold',1);sub.minimumScaleFactor=.76;
    w.addSpacer();
    const foot=w.addStack();foot.addSpacer();tx(foot,KEY==='boxing'?'複数公式sourceを照合':'公式更新を自動反映',MEDIUM_UI.status,new Color(C.muted),'semibold');foot.addSpacer();
  }else{
    const main=w.addStack();main.centerAlignContent();main.addSpacer();
