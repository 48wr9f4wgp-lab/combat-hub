...
// v7.22.8-github — hydrate sparse live candidates from verified metadata and preserve discipline labels; visuals frozen

(async()=>{
const VERSION='7.22.8-github';
...
function trustedFutureMatch(ev){if(!ev)return null;const src=eventSourceKey(ev),t=new Date(ev.startAt).getTime();return trustedFutureEvents().find(x=>{const xs=eventSourceKey(x),xt=new Date(x.startAt).getTime();if(src&&xs&&src===xs)return true;if(Number.isFinite(t)&&Number.isFinite(xt)&&Math.abs(t-xt)<=6*3600000&&validOrgName(ev.name||x.name))return true;return sameEventIdentity(ev,x);})||null;}
function enrichTrustedEventMeta(ev){if(!ev)return ev;const t=trustedFutureMatch(ev);if(!t)return ev;const rawName=stripHTML(ev.name||''),generic=KEY==='ufc'&&genericEventIdentityName(rawName),location=shortLoc(ev.location||'')||shortLoc(t.location||'');const hasMain=!!(ev.main?.a&&ev.main?.b&&ev.main.a!=='対戦カード'&&ev.main.b!=='発表待ち');return{...ev,name:generic||!rawName?(t.name||ev.name):ev.name,location:location||'',displayDate:ev.displayDate||t.displayDate||null,timeTba:typeof ev.timeTba==='boolean'?ev.timeTba:!!t.timeTba,main:hasMain?ev.main:(t.main?{...t.main}:ev.main),support:(ev.support||[]).length?ev.support:(t.support?[...t.support]:ev.support),cardTba:hasMain?!!ev.cardTba:(t.main?false:!!ev.cardTba)};}
...
const KNOWN_EVENT_CARD_REFRESH_MS=30*60*1000,CARD_POLICY_VERSION=8,LARGE_NEXT_POLICY_VERSION=2;
