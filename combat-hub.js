...
// v7.21.0-github — UFC/K-1 live-card freshness hardening; visuals frozen

(async()=>{
const VERSION='7.21.0-github';
...
async function refreshKnownRollforwardEvent(data){if(!data?.source||!supportsLiveCardRefresh())return data;for(const source of eventDetailSources(data)){try{const html=await reqText(source,8),pairs=currentPagePairs(html);if(!pairs.length)continue;const main={...(data.main||{}),a:pairs[0].a,b:pairs[0].b},support=pairs.slice(1,5).map((p,i)=>({label:i?'FEATURED':'CO-MAIN',a:p.a,b:p.b}));return{...data,source:data.source||source,main,support,cardTba:false,posterURL:metaImage(html,source)||data.posterURL||null,cardRefreshed:true};}catch(_){}}return data;}
