...
// v7.13.0-github — Large readability / hierarchy pass; BOXING cache-only path preserved

(async()=>{
const VERSION='7.13.0-github';
...
const SNAPSHOT={
  ...
  rizin:{startAt:'2026-10-03T14:00:00+09:00',location:'長崎',name:'RIZIN LANDMARK 16 in NAGASAKI',main:{a:'堀江圭功',b:'宇佐美正パトリック',context:'RIZIN MMA 71kg'},support:[{label:'CO-MAIN',a:'ビクター・コレスニック',b:'松嶋こよみ'},{label:'FEATURED',a:'芦澤竜誠',b:'井上聖矢'}],source:'https://jp.rizinff.com/_ct/17857720'},
  one:{startAt:'2026-09-11T22:30:00+09:00',location:'バンコク',name:'ONE Friday Fights 170',main:{a:'Yodlekpet Or Atchariya',b:'Pompet Pongsuphan PK',context:'フライ級ムエタイ'},support:[{label:'CO-MAIN',a:'Ayad Albadr',b:'Kongchai Chanaidonmueang'},{label:'FEATURED',a:'Yodthewin Mor Rajabhatmubanchombueng',b:'Otis Waghorn'}],source:'https://www.onefc.com/events/one-friday-fights-170/'},
...
function currentPagePairs(html){if(!html)return[];if(KEY==='one'){const out=[],seen=new Set(),chunks=String(html).split('<tr class="vs">').slice(1);for(const chunk of chunks){const row=chunk.split('</tr>')[0],titles=[...row.matchAll(/title="([^"]+)"/g)].map(m=>cleanName(m[1]));if(titles.length<2)continue;const a=titles[0],b=titles[1],k=a+'|'+b;if(a&&b&&!seen.has(k)){seen.add(k);out.push({a,b});}}if(out.length)return out.slice(0,6);}return fightPairs(html);}
...
function renderLargeMainName(box,name){const parts=mainNameParts(jpDisplay(name));for(const part of parts){const t=tx(box,part,16.0,new Color(C.text),'black',1);t.minimumScaleFactor=.62;t.centerAlignText();}return parts;}
...
const D=await loadData(),ctx=await heroContext(D);writeRuntimeAudit(D,ctx);const w=new ListWidget();const IS_LARGE=config.widgetFamily==='large';const BOXING_LARGE=IS_LARGE&&KEY==='boxing';const NEXT=BOXING_LARGE?null:(IS_LARGE?await loadLargeNext(D):null);const NEXT_POSTER=IS_LARGE&&NEXT&&!BOXING_LARGE?await eventPoster(NEXT):null;if(IS_LARGE){w.setPadding(14,16,12,16);if(BOXING_LARGE){if(ctx.poster)w.backgroundImage=ctx.poster;else w.backgroundGradient=gradient();renderLarge(w,D,ctx,NEXT,null);}else{try{w.backgroundImage=largeBackground(ctx);renderLarge(w,D,ctx,NEXT,NEXT_POSTER);}catch(_){w.backgroundColor=new Color('#020305');renderLarge(w,D,{a:{name:D.main.a,image:null},b:{name:D.main.b,image:null},poster:null},NEXT,null);}}}else{w.setPadding(10,14,8,14);
